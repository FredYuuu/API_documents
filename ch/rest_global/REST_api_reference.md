...
**请务必在`header`中设置`user agent`为：<br>**
`User-Agent`: `Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.71 Safari/537.36`

**文档中涉及到的账户ID`account-id`需要用[GET /v1/account/accounts](#get-v1accountaccounts)获取**

**`symbol` 规则：<br>** 基础币种+计价币种。如`BTC/USDT`，`symbol`为`btcusdt`；`ETH/BTC`， `symbol`为`ethbtc`。以此类推
...
| 市场行情       | [GET /market/detail](#get-marketdetail-获取-market-detail-24小时成交量数据)  | GET | 滚动24小时交易聚合行情(单个symbol)  |N |Y|
| 交易品种信息       | [GET /v1/common/symbols](#get-v1commonsymbols-查询pro站支持的所有交易对及精度)  | GET | 交易品种的计价货币和报价精度  |N |Y|
| 交易品种信息       | [GET /v1/common/currencys](#get-v1commoncurrencys-查询pro站支持的所有币种)  | GET | 交易币种列表  |N |Y|
...
|账户信息	|[GET /v1/account/accounts/{account-id}/balance](#get-v1accountaccountsaccount-idbalance-查询pro站指定账户的余额)|GET|	查询指定账户余额	|Y|Y|
|交易	|[POST/v1/order/orders/place](#post-v1orderordersplace-pro站下单)|POST|	下单	|Y|Y|
...
|杠杆交易	|[POST /v1/dw/transfer-in/margin](#post-v1dwtransfer-inmargin--现货账户划入至借贷账户)|POST|从币币交易账户划转至杠杆账户|Y|N|
|杠杆交易	|[POST /v1/dw/transfer-out/margin](#post-v1dwtransfer-outmargin--借贷账户划出至现货账户)|POST|从杠杆账户划转至币币交易账户|Y|N|
|杠杆交易	|[POST /v1/margin/orders](#post-v1marginorders-申请借贷)	|POST|申请借贷|Y|N|
|杠杆交易	|[POST /v1/margin/orders/{order-id}/repay](#post-v1marginordersorder-idrepay-归还借贷)	|POST|归还借贷|Y|N|
|杠杆交易	|[GET /v1/margin/loan-orders](#get-v1marginloan-orders--借贷订单)	|GET|查询借贷记录|Y|N|
|杠杆交易	|[GET /v1/margin/accounts/balance](#get-v1marginaccountsbalance-借贷账户详情)	|GET|查询杠杆账户余额|Y|N|
|ETF换入换出	|[GET /etf/swap/config](#get-etfswapconfig)|GET|ETF换入换出的基本信息，ETF换入换出状态，以及ETF的成分结构。|Y|N|
|ETF换入换出	|[POST/etf/swap/in](#post-etfswapin)|POST|用户可以通过该接口换入一定数量的ETF.|Y|N|
|ETF换入换出	|[POST/etf/swap/out](#post-etfswapout)|POST|用户可以通过该接口换出一定数量的ETF.|Y|N|
|ETF换入换出	|[GET/etf/list](#get-etflist)|GET|ETF换入换出操作的明细记录。最多返回 100 条记录。|Y|N|
|ETF换入换出	|[GET/quotation/market/history/kline](#get-quotationmarkethistorykline-获取etf净值)|GET|ETF净值的K线|N|N|
|母子账号|[POST /v1/subuser/transfer](#post-v1subusertransfer-母账户执行母子账户之间的划转)|POST|母账号执行母子账户之间的划转|Y|N|
|母子账号|[GET /v1/subuser/aggregate-balance](#get-v1subuseraggregate-balance)|GET|母账号查询所有子账号各币种资产累加余额|Y|N|
|母子账号|[GET /v1/account/accounts/{sub-uid}](#get-v1accountaccountssub-uid)|GET|母账号查询某个子账号的各币种和各账户类型的余额|Y|N|
...
| price-precision | true | string | 价格精度位数（0为个位，ethbtc, etcbtc, bchbtc, ltcbtc 除外，这四个交易对的精度固定为4位 ） |      |
